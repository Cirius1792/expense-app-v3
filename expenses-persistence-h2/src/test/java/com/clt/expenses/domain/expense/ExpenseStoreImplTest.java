...
class ExpenseStoreImplTest {
    ...
    void test_retrieve_expenses_by_page(int numElements) {
        IntStream.range(0, numElements * 3)
                .forEach($ -> expenseRepository.save(buildNewExpenseEntity()).as(StepVerifier::create).expectNextCount(1).verifyComplete());
        expenseRepository.findByGroupId(GROUP_ID, Pageable.ofSize(numElements))
                .as(StepVerifier::create)
