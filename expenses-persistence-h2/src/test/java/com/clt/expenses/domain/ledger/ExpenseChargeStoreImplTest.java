...
class ExpenseChargeStoreImplTest {
    ...
    private static final ExpenseCharge EXPENSE_CHARGE_GROUP_A = ImmutableExpenseCharge.builder()
            .id("1")
            .expense("e-id")
            .debtor("d-id")
            .creditor("c-id")
            ...
            .build();
    private static final ExpenseCharge EXPENSE_CHARGE_GROUP_B = ImmutableExpenseCharge.builder()
            .id("2")
            .expense("e-id")
            .debtor("d-id")
            .creditor("c-id")
    ...
    @Autowired
    ExpenseChargeRepository expenseChargeRepository;
    @Autowired
    ExpenseChargeStore store;
    ...
    void store_expense_charge_test() {
        store.store(EXPENSE_CHARGE_GROUP_A)
                .as(StepVerifier::create)
                .expectNext(EXPENSE_CHARGE_GROUP_A)
                .verifyComplete();

        expenseChargeRepository.findById(EXPENSE_CHARGE_GROUP_A.id())
                .as(StepVerifier::create)
                .expectNextCount(1)
                .verifyComplete();
    }
    ...
    void retrieve_existing_expense_by_id_test() {
        databaseClient.sql(INSERT_STATEMENT)
        ...
        store.retrieve(EXPENSE_CHARGE_GROUP_A.id())
                .as(StepVerifier::create)
                .expectNext(EXPENSE_CHARGE_GROUP_A)
                .verifyComplete();
    }
    ...
    void retrieve_existing_expense_by_person_and_group_test() {
        ...
        store.retrieveBy(EXPENSE_CHARGE_GROUP_A.debtor(), EXPENSE_CHARGE_GROUP_A.groupId())
                .as(StepVerifier::create)
                .expectNext(EXPENSE_CHARGE_GROUP_A)
                .verifyComplete();

        store.retrieveBy(EXPENSE_CHARGE_GROUP_B.debtor(), EXPENSE_CHARGE_GROUP_B.groupId())
                .as(StepVerifier::create)
