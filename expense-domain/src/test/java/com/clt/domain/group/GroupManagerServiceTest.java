...
class GroupManagerServiceTest {
    ...
    @DisplayName("When creating a group " +
            "Then the owner of the group is also a member of the group")
    @Test
    void owner_of_the_group_should_be_a_member(){
        Group actual = service.create(GROUP_NAME, GROUP_OWNER);
        Assertions.assertTrue(actual.members().contains(GROUP_OWNER), "The owner of the group should also be a member");

    }

    @DisplayName("Given a group and a user not in the group " +
