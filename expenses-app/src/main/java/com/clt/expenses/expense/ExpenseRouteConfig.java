...
import com.clt.expenses.expense.ExpenseRouter;
import com.clt.expenses.expense.ExpenseMapper;
...
public class ExpenseRouteConfig {
    ...
    @Bean
    AddExpenseUseCase addExpenseUseCase(GroupStore groupStore, PersonStore personStore,
                                        ExpenseFactory expenseFactory, ExpenseStore expenseStore){
        return new AddExpenseUseCase(personStore, groupStore, expenseFactory, expenseStore);
    }

    @Bean
    RouterFunction<ServerResponse> expensesRouter(ExpenseMapper expenseMapper, AddExpenseUseCase addExpenseUseCase){
